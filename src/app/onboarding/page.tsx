...
export default function OnboardingPage() {
  ...
  const [selected, setSelected] = useState(["ana", "carlos", "sofia"]);
  ...
  function finish() { setError(""); startTransition(async () => { try { await onboardOrganization({ ...company, employees: selected }); } catch (cause) { setError(cause instanceof Error ? cause.message : "Não foi possível criar a empresa."); } }); }
  ...
  return <main className="onboarding"><div className="onboarding-wrap"><Logo /><div className="stepper">{[1, 2, 3].map((item) => <span className={item <= step ? "done" : ""} key={item} />)}</div>
    ...
    {step === 2 && <section><p className="eyebrow">Etapa 2 de 3</p><h1>Escolha os primeiros funcionários</h1><p className="subtitle" style={{ marginBottom: 18 }}>Seu plano inclui até 3. Você poderá mudar a equipe depois.</p><div className="employee-grid">{employees.slice(0, 5).map((employee) => <button type="button" className="card employee-card" style={{ textAlign: "left", borderColor: selected.includes(employee.id) ? "rgba(255,244,92,.5)" : undefined }} onClick={() => toggle(employee.id)} key={employee.id}><div className="employee-top"><Avatar initials={employee.initials} color={employee.color} size="lg" /><div className="employee-title"><h3>{employee.name}</h3><p>{employee.role}</p></div>{selected.includes(employee.id) && <span className="metric-icon"><Check size={14} /></span>}</div><p className="subtitle">{employee.description}</p></button>)}</div></section>}
