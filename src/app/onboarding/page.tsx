...
export default function OnboardingPage() {
  ...
  const [selected, setSelected] = useState(backendEnabled ? ["ana", "carlos", "sofia"] : ["ana", "carlos"]);
  ...
  return <main className="onboarding"><div className="onboarding-wrap"><Logo /><div className="stepper">{[1, 2, 3].map((item) => <span className={item <= step ? "done" : ""} key={item} />)}</div>
    ...
    {step === 2 && <section><p className="eyebrow">Etapa 2 de 3</p><h1>Escolha os primeiros funcionários</h1><p className="subtitle" style={{ marginBottom: 18 }}>{backendEnabled ? "Seu plano inclui até 3. Você poderá mudar a equipe depois." : "Nesta validação, Ana e Carlos já possuem áreas operacionais. Os demais funcionários serão liberados nas próximas etapas."}</p><div className="employee-grid">{employees.slice(0, 5).map((employee) => { const unavailable = !backendEnabled && !["ana", "carlos"].includes(employee.id); return <button type="button" disabled={unavailable} className="card employee-card" style={{ textAlign: "left", borderColor: selected.includes(employee.id) ? "rgba(255,244,92,.5)" : undefined, opacity: unavailable ? .45 : 1 }} onClick={() => toggle(employee.id)} key={employee.id}><div className="employee-top"><Avatar initials={employee.initials} color={employee.color} size="lg" /><div className="employee-title"><h3>{employee.name}</h3><p>{unavailable ? "Disponível em breve" : employee.role}</p></div>{selected.includes(employee.id) && <span className="metric-icon"><Check size={14} /></span>}</div><p className="subtitle">{employee.description}</p></button>; })}</div></section>}
