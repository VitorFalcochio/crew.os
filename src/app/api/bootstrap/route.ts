...
export async function GET() {
  try {
    ...
    const [organization, employees, tasks, approvals, activities, integrations, subscription, profile] = await Promise.all([
    ...
    const failed = [organization, employees, tasks, approvals, activities, integrations, subscription, profile].find((result) => result.error);
    if (failed?.error) throw failed.error;
    return Response.json({ account: { userId: user.id, email: user.email, name: profile.data?.full_name ?? user.user_metadata.full_name ?? user.email, organization: organization.data, role: membership.role }, employees: employees.data, tasks: tasks.data, approvals: approvals.data, activities: activities.data, integrations: integrations.data, subscription: subscription.data }, { headers: { "Cache-Control": "private, no-store" } });
