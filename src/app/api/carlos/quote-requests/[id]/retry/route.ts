...
export async function POST(request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  try { assertLocalCollectionApi(request); actionId = (await context.params).id; const body = await request.json().catch(() => ({})) as { actor?: unknown }; const actor = typeof body.actor === "string" && body.actor.trim() ? body.actor.trim().slice(0, 120) : "Gestor local"; const action = await beginLocalSupplierQuoteRequestSend(actionId, { actor, retry: true }); if (action.status === "sent") return Response.json({ data: action }); started = true; const result = await sendGoogleExternalEmail(action); return Response.json({ data: await markLocalSupplierQuoteRequestSent(actionId, result) }); }
